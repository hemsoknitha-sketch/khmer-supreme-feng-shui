...
class CurriculumEngine:
    ...
    def get_lesson(self, lesson_id: int) -> Optional[Dict[str, Any]]:
        ...
        Fully comprehensive, authentic, and beautifully styled with extensive masterclass depth for all 5 points.
        """
        ...
        # 10 Distinct Sub-Lesson Archetypes with Deep Multi-Paragraph Content
        archetype_titles = [
        ...
        # 1. Classical Treatise & Philosophical Exegesis (📜 ១. គម្ពីរ និងទស្សនវិជ្ជាគ្រឹះ)
        classical_rule = (
            f"📖 **ដកស្រង់ចេញពីគម្ពីរដើម៖** {t['treatise']}\n"
            f"📜 **អត្ថបទដើមចិនបុរាណ៖** 『{t['formula']}』\n\n"
            f"🔍 **ការពន្យល់ន័យធៀប និងទស្សនវិជ្ជាគ្រឹះ៖**\n"
            f"យោងតាមក្រឹត្យក្រមធម្មជាតិបុរាណចិន ថាមពលក្នុងសកលលោកកើតចេញពីភាពទទេ (Wuji) បង្កើតបានជាយិនយ៉ាង (Taiji) "
            f"ហើយបន្តបែកចែកជាធាតុទាំង ៥ (Wu Xing) និងត្រីក្រាមទាំង ៨ (Ba Gua)។ "
            f"សម្រាប់ប្រធានបទ '{t['name_kh']}' មេរៀននេះបង្រៀនឱ្យយើងយល់ច្បាស់ថា៖ {t['essence']} "
            f"នេះមិនមែនជាជំនឿអរូបីឡើយ ប៉ុន្តែជាវិទ្យាសាស្ត្របរិស្ថាន និងរូបវិទ្យាដែនម៉ាញេទិច "
            f"ដែលសម្របសម្រួលរវាង មេឃ (Tian - ពេលវេលា), ផែនដី (Di - លំហរស់នៅ), និងមនុស្ស (Ren - ជោគជតារាសី) "
            f"ឱ្យស្ថិតក្នុងលំនឹងថាមពលខ្ពស់បំផុត។"
        )

        # 2. Precision Mathematics & Luopan 24 Mountains Calibration (📐 ២. រូបមន្តគណិតវិទ្យា & ឡូប៉ាន ២៤ ភ្នំ)
        formula = (
            f"📐 **សមីការគណិតវិទ្យាហុងស៊ុយ៖**\n"
            f"• មាត្រដ្ឋានឡូប៉ាន៖ `360° / 24 ភ្នំ = 15.0° ក្នុង ១ ភ្នំ`\n"
            f"• ម៉ាទ្រីសលួស៊ូ (Luo Shu Sum Constant) = `15` (ផលបូកគ្រប់ជួរដេក ជួរឈរ និងអង្កត់ទ្រូង)\n"
            f"• ក្រឹត្យក្រមទិសដៅ៖ `{t['formula']}`\n\n"
            f"🧭 **ការកំណត់មុំអង្សាជាក់ស្តែង (Degree Calibration):**\n"
            f"១. **កណ្តាលភ្នំ (Zheng Shan 正山):** ស្ថិតក្នុងកម្រិត ±4.5° ពីចំណុចកណ្តាលនៃភ្នំនីមួយៗ (ថាមពលបរិសុទ្ធ 100% Qi)\n"
            f"២. **ភ្នំលំអៀង (Jian Xiang 兼向):** ស្ថិតក្នុងកម្រិតពី 4.5° ដល់ 6.0° (ត្រូវប្រើក្បួនផ្កាយជំនួស Ti Gua)\n"
            f"៣. **ខ្សែបន្ទាត់មរណៈ (Kong Wang 空亡):** ហាមដាច់ខាតកុំឱ្យធ្លាក់លើមុំ `0°, 45°, 90°, 135°, 180°, 225°, 270°, 315° (±1.5°)` ព្រោះជាខ្សែបន្ទាត់ច្របូកច្របល់ដែនម៉ាញេទិច (Great Void Line)។"
        )

        # 3. Exhaustive 7 Core Pillars Matrix Analysis (🏛️ ៣. ការវិភាគ ៧ សសរស្តម្ភ AGI)
        geo_a = f"{t['geo']} រៀបចំរចនាសម្ព័ន្ធដីតាមក្បួន 'សត្វសួគ៌ាទាំង ៤' (អណ្តើកខ្មៅទ្រពីក្រោយ, នាគរាជឆ្វេងខ្ពស់, ខ្លាស្តាំរាបទាប, សត្វស្លាបក្រហមមុខទូលាយ)។"
        qi_a = f"{t['qi']} រក្សាល្បឿនខ្យល់ក្រោម 3.5 km/h (Gentle Breeze) ដើម្បីឱ្យថាមពល Sheng Qi ប្រមូលផ្តុំនៅចំណុចកណ្តាល Taiji មិនឱ្យបែកខ្ញែកឡើយ។"
        time_a = f"{t['time']} យុគទី ៩ គ្រប់គ្រងដោយតារា ៩ ស្វាយ (Li Fire) ត្រូវរៀបចំទិសខាងត្បូងជា Zheng Shen (ត្រូវការភ្នំ/ជញ្ជាំង) និងទិសខាងជើងជា Ling Shen (ត្រូវការទឹកហូរ/ផ្លូវទូលាយ)។"
        bazi_s = f"{t['bazi']} ផ្សារភ្ជាប់ជាមួយធាតុឱសថ Yong Shen (用神) នៃ Day Master ដើម្បីជួយកែប្រែចំណុចខ្វះខាតក្នុងជោគជតា ៤ សសរស្តម្ភ។"

        # 4. Actionable Step-by-Step Practical Layout & 5 Elements Remedies (💡 ៤. ការអនុវត្តជាក់ស្តែង & ដំណោះស្រាយ)
        practical_remedy = (
            f"🏠 **១. ដំណោះស្រាយក្នុងលំនៅឋាន (Residential Layout):**\n"
            f"• **ទ្វារធំ (Main Door):** {t['remedy']} ធានាថាមុខទ្វារមានពន្លឺគ្រប់គ្រាន់ និងគ្មានឧបសគ្គបាំងផ្លូវខ្យល់។\n"
            f"• **បន្ទប់គេង (Master Bed):** ក្បាលគ្រែត្រូវផ្អែកនឹងជញ្ជាំងរឹងមាំ (គ្មានបង្អួចពីក្រោយ) តម្រង់ទៅទិស Tian Yi ឬ Yan Nian របស់មេគ្រួសារ។\n"
            f"• **ផ្ទះបាយ (Kitchen Stove):** ដាក់ចង្ក្រាននៅទីតាំងដកធាតុអាក្រក់ បែរមុខទៅទិសល្អ (坐凶向吉) គម្លាតពីកន្លែងលាងចានយ៉ាងតិច 60cm។\n\n"
            f"🏢 **២. ដំណោះស្រាយក្នុងអាជីវកម្ម (Commercial Wealth Gateway):**\n"
            f"• **តុថៅកែ/តុគិតលុយ:** ស្ថិតនៅទីតាំងបញ្ជា (Command Position) មើលឃើញច្រកចូលទាំងមូល ផ្អែកជញ្ជាំងរឹងមាំ មិននៅក្រោមធ្នឹម។\n"
            f"• **រៀបចំផ្លូវទឹកស្រូបទ្រព្យ:** ដាក់អាងទឹកហូរ ឬទឹកវិលជុំនៅទិសខាងជើង (Ling Shen) ឬទិសតារាទឹកលេខ ៩ ដើម្បីទាក់ទាញលំហូរអតិថិជន។\n\n"
            f"🌿 **៣. វិធីបន្សាបតាមធាតុទាំង ៥ (Five Elements Remedies):**\n"
            f"• បើជួបថាមពលកាច (ដីកាច ២ ខ្មៅ/៥ លឿង)៖ ប្រើកណ្តឹងខ្យល់លោហធាតុ ៦ បំពង់ ឬកាក់ ៦ កាក់ (Metal weakens Earth)។\n"
            f"• បើជួបជម្លោះមាត់ក (ឈើ ៣ ខៀវ)៖ ប្រើពន្លឺភ្លើងពណ៌ក្រហម ឬវត្ថុធាតុភ្លើង (Fire burns Wood)។\n"
            f"• បើជួបថាមពលស្ទះ៖ ប្រើរុក្ខជាតិបៃតងស្លឹកមូល ឬទឹកហូរស្រាលៗដើម្បីបន្សាប។"
        )

        # 5. Critical Taboos & Warning Diagnostics (⚠️ ៥. ចំណុចហាមឃាត់ & សញ្ញាព្រមាន)
        taboo_warning = (
            f"🚨 **ចំណុចហាមឃាត់ដាច់ខាត (Strict Taboos):**\n"
            f"• {t['taboo']}\n"
            f"• ហាមទ្វារធំ និងទ្វារក្រោយចាក់ចំគ្នាត្រង់ភ្លឹង (穿堂煞 Chuan Tang Sha) នាំឱ្យហូរធ្លាយទ្រព្យសម្បត្តិ និងរកលុយមិនសល់។\n"
            f"• ហាមដាក់កញ្ចក់ឆ្លុះចំគ្រែគេង ឬចំទ្វារមុខផ្ទះ (នាំឱ្យកើតជំងឺផ្លូវចិត្ត និងបណ្តេញទេវតាសំណាងចេញក្រៅ)។\n"
            f"• ហាមដាក់ចង្ក្រានហ្គាសនៅទិសពាយព្យ (NW) ដាច់ខាត (ភ្លើងដុតទ្វារមេឃ នាំឱ្យខូចសុខភាព និងអំណាចមេគ្រួសារ)។\n\n"
            f"🩺 **រោគសញ្ញាព្រមាន (Warning Diagnosis Signs):**\n"
            f"ប្រសិនបើគេហដ្ឋានមានការរៀបចំខុសក្បួននេះ សមាជិកគ្រួសារអាចប្រឈមមុខនឹង៖ ឈឺក្បាលរ៉ាំរ៉ៃ, គេងមិនលក់, ជម្លោះពាក្យសម្តីញឹកញាប់, ឬការខាតបង់ប្រាក់កាសដោយមិនដឹងមូលហេតុ។\n\n"
            f"🛠️ **វិធីសង្គ្រោះបន្ទាន់ (Emergency Rectification):**\n"
            f"ប្រសិនបើមិនអាចរុះរើជញ្ជាំងបានភ្លាមៗ ត្រូវប្រើផ្ទាំងរនាំងបាំង (Divider Screen) ឬដាក់ដើមឈើលម្អ និងកែតម្រូវមុំកញ្ចក់ជាបន្ទាន់។"
        )
